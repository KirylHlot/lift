...
window.onload = function(){
	setMaxHeightAllBlocks ("content_res");
}
...
$(document).ready(function() {
   $(".header_form").submit(function() {
   	if(checkInputs(this)){
         ...
         $(this).find("input").val("");
         $(".header_form").addClass('d-none');
         $(".header_form_alert").removeClass('d-none');;
      }).fail(function() {
...
$(document).ready(function() {
   $(".appeal_form").submit(function() {
      ...
      $.ajax({
         type: "POST",
         url: "appeal_form_mail.php",
         data: $(this).serialize()
      }).done(function() {
				$(this).find("input").val("");
				hide_form ("appeal_form");
				$(".appeal_form_alert").removeClass('d-none');
      }).fail(function() {
        alert('Ошибка соединения');
      });
		}
...
$(document).ready(function() {
   $(".consultation_form").submit(function() {
   	if(checkInputs(this)){
	      $.ajax({
	         type: "POST",
	         url: "consultation_form_mail.php",
	         data: $(this).serialize()
	      }).done(function() {
					$(this).find("input").val("");
					hide_form ("consultation_form");
					$(".consultation_form_alert").removeClass('d-none');
	      }).fail(function() {
	        alert('Ошибка соединения');
	      });
			}
...
//сделал в верстке криво, поэтому так
function hide_form (formClassName) {
	let input_wrapper_mass = document.querySelector('.' + formClassName).getElementsByClassName('input_wrapper');
	
	for (var i = 0; i < input_wrapper_mass.length; i++) {
		input_wrapper_mass[i].classList.add('d-none');
	}
	document.querySelector('.' + formClassName).querySelector('.button').classList.add('d-none');
	
	if(document.querySelector('.' + formClassName).querySelector('.under_button')){
		document.querySelector('.' + formClassName).querySelector('.under_button').classList.add('d-none');
	}
...
$(document).ready(function() {
	...
	$('.anim_1').addClass("hidden").viewportChecker({
		classToAdd: 'visible animated initial bounceIn',
		offset: 0,
		repeat: true
	});	
